...
void ADC_PTB1_config_basica(uint8_t trigger) {
	...
	// Habilita e seleciona trigger para ADC0
	SIM_SOPT7 &= ~SIM_SOPT7_ADC0PRETRGSEL_MASK;
	SIM_SOPT7 |= (SIM_SOPT7_ADC0ALTTRGEN_MASK
	 | SIM_SOPT7_ADC0TRGSEL(trigger)) ;

	//Configura pino PTB1 como canal de conversao:
	PORTB_PCR1 &= ~PORT_PCR_MUX(0b111);	// MUX(0b0000) -> ADC0_SE9
...
void ADC_Cal (ADC_MemMapPtr end) {
	...
	//Muda para disparo por software antes de calibrar (pag. 495 do manual de referencia)
