...
void TPM1TPM2_PTB0PTB18PTB19_config_basica () {
	...
	SIM_SCGC6 |= (SIM_SCGC6_TPM1_MASK |		// Habilita o clock de TPM1
		SIM_SCGC6_TPM2_MASK);				// Habilita o clock de TPM2

	//Habilitar os sinais de relogio para PORTB
	...
	PORTB_PCR0 |= PORT_PCR_MUX(0x3);			// PTB0 associado a TPM1_CH0 (Cooler)
	PORTB_PCR18 |= PORT_PCR_MUX(0x3);			// PTA18 associado a TPM2_CH0 (Led R)
	PORTB_PCR19 |= PORT_PCR_MUX(0x3);			// PTE19 associado a TPM2_CH1 (Led G)
}

void TPM_config_especifica (uint8_t x, uint16_t mod, uint8_t trigger, uint8_t crot, uint8_t csoo, uint8_t csot,
		uint8_t dma, uint8_t cpwms, uint8_t ps,uint8_t cmod) {
	...
	TPM[x]->SC |= TPM_SC_CMOD(0);			// Desativa o contador LPTPM
	return;
}
...
void TPM_habilitaContagem(uint8_t x){
	TPM[x]->SC |= TPM_SC_CMOD(1);
}
