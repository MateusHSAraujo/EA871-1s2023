...
int main(void){
	...
	UART0_habilitaNVICIRQ12(0b0);//Habilita tratamento de interrupcoes de UART0
	UART0_habilitaInterruptRxTerminal();//Habilita interrupcoes de recebimento
	
	//Incializa pilha do LCD
	ISR_inicializaBP();
	
	//Cria bitmap e configura bitmap para ser usado no fim do jogo
	uint8_t pontaDeSeta[8]={0b10000,0b11000,0b11100,0b11110,0b11110,0b11100,0b11000,0b10000};
	uint8_t corpoDeSeta[8]={0b00000,0b00000,0b00000,0b11111,0b11111,0b00000,0b00000,0b00000};
	char eComAcento[8]={0b00010,0b00100,0b00000,0b01110,0b10001,0b11111,0b10000,0b01110};
	GPIO_escreveBitmapLCD (0x00, corpoDeSeta);
	GPIO_escreveBitmapLCD (0x01, pontaDeSeta);
	...
	for(;;){
		switch(ISR_LeEstado()){
					...
					case PREPARA_LCD:
						...
						GPIO_escreveStringLCD(0x41,(uint8_t *) "    em 5 s     ");
